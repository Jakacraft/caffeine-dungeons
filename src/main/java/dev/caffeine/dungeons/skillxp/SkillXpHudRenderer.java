...
public final class SkillXpHudRenderer {
    ...
    private static final int VBAR_BORDER = 1;
    ...
    private static final int VBAR_TRACK = 0xFF1B1B1B;
    private static final int VBAR_BORDER_COLOR = 0xFF000000;
    ...
    private static void drawVanillaStyle(DrawContext context, SkillXpEntry entry, float alpha, long now) {
        ...
        context.fill(0, barY, VBAR_W, barY + VBAR_H + VBAR_BORDER * 2, (VBAR_BORDER_COLOR & 0x00FFFFFF) | a);
        int trackX = VBAR_BORDER;
        int trackY = barY + VBAR_BORDER;
        context.fill(trackX, trackY, trackX + VBAR_W - VBAR_BORDER * 2, trackY + VBAR_H, (VBAR_TRACK & 0x00FFFFFF) | a);

        float pulse = entry.pulse(now);
        int fillRgb = pulse > 0f ? blend(VBAR_FILL, 0xFFFFFF, pulse * 0.4f) : (VBAR_FILL & 0x00FFFFFF);
        int fillColor = (fillRgb & 0x00FFFFFF) | a;
        int fillW = Math.round((VBAR_W - VBAR_BORDER * 2) * entry.displayedProgress);
        if (fillW > 0) {
            context.fill(trackX, trackY, trackX + fillW, trackY + VBAR_H, fillColor);
        }
    }
